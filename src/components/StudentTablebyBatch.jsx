  ...
  const StdRow = ({ name, phone, date, email, batch, sNo }) => {
    return (
      <tr className="flex w-full border-b justify-around">
        ...
        <td
          title={name}
          className="py-[16px] w-[30%] overflow-hidden whitespace-nowrap text-ellipsis text-center"
        >
          {name}
        </td>
        <td className="py-[16px] w-[10%] text-center">
          {assignBatchTable ? "-" : batch}
        </td>
        ...
        <td className="py-[16px] items-center flex justify-center w-[15%] gap-[10px]  text-center">
          {date}{" "}
          {assignBatchTable ? (
            <button className="bg-green-400 text-[12px] font-semibold rounded-[16px] text-white px-[8px] py-[5px]">
              Assign
            </button>
          ) : (
            ""
  ...
  return (
    <table className="w-full overflow-auto scroll-style rounded-[16px] h-full flex flex-col">
      ...
      <tbody className="mt-[50px]">
        {(assignBatchTable ? filteredData : studentDataByBatch).map(
          ({ name, batch, phone, email, date }, idx) => (
            <StdRow
              key={idx + 1}
              sNo={idx + 1}
              name={name}
