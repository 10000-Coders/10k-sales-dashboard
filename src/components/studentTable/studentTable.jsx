'use client';
import {LeftArrow, OrangeLable, RightArrow, IconCalender, UserIcon} from '@/shared/svgImages/tableImages';
      ...
      <div className="w-full gap-[24px] items-center flex text-[16px] font-[700] ">
        {filterPaths.includes(path) && (
          <div className="ml-[35%] flex  border items-center shadow px-[10px] py-[5px] rounded-[12px]">
            {filterPaths[1] === path ? <IconCalender /> : filterPaths[0] === path ? <UserIcon /> : ''}

            <Select
              options={filterPaths[1] === path ? MonthOptions : filterPaths[0] === path ? PersonOptions : ''}
              placeholder={`${filterPaths[1] === path ? 'Month' : filterPaths[0] === path ? 'Person' : ''}`}
